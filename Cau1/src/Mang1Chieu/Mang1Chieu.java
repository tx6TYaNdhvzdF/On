...
public class Mang1Chieu {
	...
	public static void XuatLeChan(int[] arr, int n) {
		System.out.println("\n\t-----CÁC PHẦN TỬ CHẲN VÀ LẺ CỦA MẢNG-------\n");
		int tong =0;
		for ( int i = 0; i < n; i++) {
			if ( arr[i] % 2 != 0 ) {
				System.out.println("Phần tử lẻ thứ: "+(i+1)+": "+arr[i]);
			}
			if ( arr[i] % 2 == 0 ) {
				System.out.println("Phần tử chẵn thứ: "+(i+1)+": "+arr[i]);
			}
			tong +=arr[i];
